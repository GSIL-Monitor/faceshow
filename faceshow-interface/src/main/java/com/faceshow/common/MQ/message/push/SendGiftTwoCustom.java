...
@Component
public class SendGiftTwoCustom implements CommandLineRunner {
    ...
    public void run(String... strings) throws Exception {
        DefaultMQPushConsumer consumer = RocketMQConfig.initConsumer(MqToppic.sendGiftTwo.toString());

        try {
            // 设置topic和标签
            consumer.subscribe(MqToppic.sendGiftTwo.toString(), null);
            ...
            consumer.registerMessageListener(new MessageListenerConcurrently() {
                public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> list, ConsumeConcurrentlyContext Context) {
                    ...
                    for (MessageExt msg : list) {
                        ...
                        giftInfoDao.inserlive_gift_send(map);
                        map.put("ITEM_ID", map.get("live_id"));//插入总表用--------
                        SendGiftProduct.SendGiftProduct(map);
                        logger.info("Two送礼物消息已经处理一条");
                    }
