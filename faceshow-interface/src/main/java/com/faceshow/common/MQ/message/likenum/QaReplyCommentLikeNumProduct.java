 ...
 * -------------------------------------------------------------- <br>
 */
public class QaReplyCommentLikeNumProduct {

    /**
     * 问答回复评论点赞数量+1或者-1操作Product
     *
     * @param commentId 问答回复评论id
     * @param likeNum   点赞数+1或者-1
     */
    public static void updateQaReplyCommentLikeNum(Integer commentId, Integer likeNum) {
        String mssage = JSON.toJSONString(new NumUtils(commentId, likeNum));
        ProductMQ.sendMessage(MqToppic.top_qaReplyCommentLikeNum, mssage);
    }
}
