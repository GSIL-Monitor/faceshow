 ...
 */
@Component
public class VideoAtPushCustom implements CommandLineRunner {
    ...
    public void run(String... strings) throws Exception {
        DefaultMQPushConsumer consumer = RocketMQConfig.initConsumer(MqToppic.top_videoAtPush.toString());

        try {
            // 设置topic和标签
            consumer.subscribe(MqToppic.top_videoAtPush.toString(), null);
            consumer.setConsumeMessageBatchMaxSize(100);
            // 程序第一次启动从消息队列头取数据
            consumer.setConsumeFromWhere(ConsumeFromWhere.CONSUME_FROM_FIRST_OFFSET);
            consumer.registerMessageListener(new MessageListenerConcurrently() {
                public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> list, ConsumeConcurrentlyContext Context) {
                    logger.info("custom 视频@好友推送消费 条数:" + list.size());
                    ...
                    for (MessageExt msg : list) {
                        ...
                        // 推送内容
                        String format = MessageFormat.format(JpushTemplet.VIDEO_AT, nickName);
                        for (PushBeanUtils pushBean : pushBeanUtils) {
                            // 把站内信存储到数据库
                            userInfoDao.insertMessage_push_info(JpushTemplet.VIDEO_AT_TYPE, userId, pushBean.getAccount(), new Date(), format);
                            ...
                            if (account.get("is_video").toString().equalsIgnoreCase("1")) {
                                ...
                                JpushPrameter.put("type_id", JpushTemplet.VIDEO_AT_TYPE.toString());
                                JpushPrameter.put("videoId", pushBean.getInfoId().toString());
