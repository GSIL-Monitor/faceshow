 ...
 */
@Component
public class QaReplyCommentLikeNumCustom implements CommandLineRunner {
    ...
    public void run(String... strings) throws Exception {
        DefaultMQPushConsumer consumer = RocketMQConfig.initConsumer(MqToppic.top_qaReplyCommentLikeNum.toString());

        try {
            // 设置topic和标签
            consumer.subscribe(MqToppic.top_qaReplyCommentLikeNum.toString(), null);
            consumer.setConsumeMessageBatchMaxSize(100);
            // 程序第一次启动从消息队列头取数据
            consumer.setConsumeFromWhere(ConsumeFromWhere.CONSUME_FROM_FIRST_OFFSET);
            consumer.registerMessageListener(new MessageListenerConcurrently() {
                public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> list, ConsumeConcurrentlyContext Context) {
                    logger.info("custom 问答回复评论点赞记录消费 条数:" + list.size());

                    for (MessageExt msg : list) {
                        String logJsonStr = new String(msg.getBody());
                        NumUtils numUtils = JSON.parseObject(logJsonStr, NumUtils.class);
                        qaReplyCommentDao.updateLikeNum(numUtils);
                        int userId = qaReplyCommentDao.selectUserIdByCommentId(numUtils.getId());
