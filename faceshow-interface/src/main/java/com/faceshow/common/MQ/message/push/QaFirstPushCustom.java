 ...
 */
@Component
public class QaFirstPushCustom implements CommandLineRunner {
    ...
    public void run(String... strings) throws Exception {
        DefaultMQPushConsumer consumer = RocketMQConfig.initConsumer(MqToppic.top_qaFirstPush.toString());

        try {
            // 设置topic和标签
            consumer.subscribe(MqToppic.top_qaFirstPush.toString(), null);
            consumer.setConsumeMessageBatchMaxSize(100);
            // 程序第一次启动从消息队列头取数据
            consumer.setConsumeFromWhere(ConsumeFromWhere.CONSUME_FROM_FIRST_OFFSET);
            consumer.registerMessageListener(new MessageListenerConcurrently() {
                public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> list, ConsumeConcurrentlyContext Context) {
                    logger.info("custom 问答评为第一名推送消费 条数:" + list.size());
                    ...
                    for (MessageExt msg : list) {
                        ...
                        // 根据问答回复id查询问答发布人id
                        Integer userId = qaInfoDao.findUserIdReplyId(pushBeanUtils.getInfoId());

                        // 向站内信插入一条数据
                        userInfoDao.insertMessage_push_info(JpushTemplet.QA_FIRST_TYPE, userId, pushBeanUtils.getAccount(), new Date(), JpushTemplet.QA_FIRST);
                        ...
                        if (map.get("is_qa").toString().equalsIgnoreCase("1")) {
                            ...
                            JpushPrameter.put("type_id", JpushTemplet.QA_FIRST_TYPE.toString());
                            JpushPrameter.put("replyId", pushBeanUtils.getInfoId().toString());

                            // 开始推送
                            JpushUtils.buildIOSParameter(JpushTemplet.QA_FIRST, JpushPrameter, pushBeanUtils.getAccount().toString());
                            JpushUtils.buildAndroidParameter(JpushTemplet.QA_FIRST, "", JpushPrameter, pushBeanUtils.getAccount().toString());
                        }
                    }
